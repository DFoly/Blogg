...
class User(UserMixin, db.Model):
    ...
    def generate_fake(count=100):
        ...
        for i in range(count):
            #generate random users + autoconfirm account
            ...
            try:
                db.session.commit()
                # small chance that will be duplicates
                #if so we rollback and do again
...
class Post(db.Model):
    ...
    def generate_fake(count=100):
        ...
        for i in range(count):
            #generate post from random user
    ...
    @staticmethod
    # initial conversion to markdown, clean removes any tags not in allowed
    # tags. linkify converts urls in plain text to proper html links
